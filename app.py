...
def draft_interface():
    ...
    tab1, tab2, tab3 = st.tabs(["**Live Draft**", "**Draft Board**", "**Settings**"])
...
def show_settings_view():
    """Show draft settings and modification options"""
    engine = st.session_state.draft_engine
    
    # Check if draft is concluded
    draft_concluded = engine.current_pick > engine.total_picks
    
    if draft_concluded:
        st.warning("⚠️ Draft has concluded. Settings cannot be modified.")
        return
    
    
    # Create two columns for different settings
    col1, col2 = st.columns([1, 1])
    
    # Column 1 - Change User Team
    with col1:
        st.subheader("Change Your Team")
        st.write("Select which team you want to control:")
        
        current_team = engine.user_position
        team_options = [f"Team {i}" for i in range(1, engine.league_size + 1)]
        
        new_team = st.selectbox(
            "Your Team:",
            range(1, engine.league_size + 1),
            index=current_team - 1,
            format_func=lambda x: f"Team {x}",
            key="settings_user_team"
        )
        
        if st.button("Update My Team", key="update_user_team"):
            engine.user_position = new_team
